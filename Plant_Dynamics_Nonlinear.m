function dX = Plant_Dynamics_Nonlinear(x, d, U, params)
    ...
    % x = [dw, Pm, Pv, PB, SoC, Tin, PAC, int_dw]
    
    dw = x(1); Pm = x(2); Pv = x(3); PB = x(4); 
    SoC = x(5); Tin = x(6); PAC = x(7); int_dw = x(8);
    ...
    dw_bound = max(dw, -0.5); % Prevent algebraic singularity
    ...
    dX(3) = (0.85 - Pv - (1/params.R)*dw) / params.Tg;
