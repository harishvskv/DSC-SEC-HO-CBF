function U_edge = Controller_DSC_SEC_HOCBF(x, d, params)
    ...
    df = x(1); Pm = x(2); Pv = x(3); PB = x(4); PAC = x(7);
    P_res = d(1); P_L0 = d(2);

    %% 1. Nonlinear Relative Degree Formulation
    df_bound = max(df, -0.5);
    P_load = P_L0 * (1 + params.D * df_bound)^params.gamma;
    f1 = (1/params.M) * (Pm + P_res + PB - PAC - P_load); % df_dot

    Pm_dot = (-Pm + Pv) / params.Tt;
    PB_dot = -(1/params.TB) * PB;
    PAC_dot = -(1/params.Tac) * PAC;
    Pload_dot = params.D * params.gamma * P_L0 * (1 + params.D * df_bound)^(params.gamma - 1) * f1;
    ...
    df_min = -0.02; % Safety Limit (pu) fBse = 50Hz
    h = df - df_min;
    ...
    % Optimize BESS and AIAC dispatch while penalizing thermodynamic deviation (for Thermal Comfort)
    H = diag([10, 500]); % Penalize AIAC usage to preserve comfort
    f_qp = [0; 0];
