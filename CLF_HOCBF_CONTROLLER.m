function [u1, u2] = CLF_HOCBF_CONTROLLER(x, d, params)
    ...
    f1 = (1/params.M) * (Pm + Pres + PB - PAC - P_load); % df_dot
    ...
    %% 2. Reference Controller (PI + Virtual Inertia->BESS)
    Kp = 1.5; Ki = 0.8; K_vi = 0.6;  %From Literature
    u1_ref = -(Kp * dw + Ki * int_df + K_vi * f1); 
    u2_ref = 0.2 + (Kp * dw + Ki * int_df); 

    %% 3. HOCBF (Safety)
    df_min = -0.02;  %fBase = 50Hz
    h = df - df_min;
    cb1 = 8; cb2 = 8; %Tuned
    A_cbf = -LgLfh;
    B_cbf = Lf2h + (cb1 + cb2) * f1 + cb1 * cb2 * h;

    %% 4. Backstepping CLF (Stability Analysis)
    c1 = 4; c2 = 4;
    z1 = df; z2 = f1 + c1 * z1;
    V = 0.5 * z1^2 + 0.5 * z2^2;
